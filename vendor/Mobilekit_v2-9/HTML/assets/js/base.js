...
const Mobilekit = {
	...
	Test: {
		...
		alertMessage:
			"Test mode has been activated. Look at the developer console!", // Alert message
	},
	//-------------------------------------------------------------------
};
//-----------------------------------------------------------------------
...
if (Mobilekit.RTL.enable) {
	...
	document
		.querySelectorAll(
			".carousel-full, .carousel-single, .carousel-multiple, .carousel-small, .carousel-slider, .story-block"
		)
		.forEach(function (el) {
...
var detectionWindowsPhone = document.querySelectorAll(
...
var multiListview = document.querySelectorAll(
	".listview .multi-level > a.item"
);
...
multiListview.forEach(function (el) {
	el.addEventListener("click", function () {
		...
		var activatedContainer = listview.querySelectorAll(
			".multi-level.active .listview"
		);
...
if (
	...
	checkDarkModeStatus === "1" ||
	pageBody.classList.contains("dark-mode-active")
) {
...
function countdownTimer(time) {
	...
	setInterval(() => {
		...
		if (r >= 0) {
			...
			document.getElementById("countDown").innerHTML =
				"<p class='alert alert-outline-warning'>The countdown is over.</p>";
		}
	}, 1000);
}
//-----------------------------------------------------------------------
...
function testMode() {
	...
	console.clear();
	console.log(
		"%cMobilekit (v" + Mobilekit.version + ")",
		"font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #1E74FD; padding: 14px 70px; margin-bottom: 16px;"
	);
	console.log(
	...
	testModeMsg(
		Mobilekit.Dark_Mode.night_mode.enable,
		"Night mode (between " +
			Mobilekit.Dark_Mode.night_mode.start_time +
			":00 and " +
			Mobilekit.Dark_Mode.night_mode.end_time +
			":00)"
	);
